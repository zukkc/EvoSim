...
void InspectorPanel::on_gui_render() {
  ...
  if (m_active == nullptr || m_active->is_dead()) {
    ...
    ImGui::Text("Agent ID: x=%f, y=%f", m_active->get_position().x, m_active->get_position().y);
    ImGui::Text("Reproduction treshold of energy: %f", m_active->get_reproduce_threshold());
    ImGui::Text("Reproduction cooldown remening: %f", m_active->get_reproduction_cooldown());
    ImGui::BeginGroup();
    draw_genome_table();
    ImGui::EndGroup();
  }
...
void InspectorPanel::draw_genome_table()
{
    ...
    static_assert(
        Genome{}.size() == input_count * neuron_count,
